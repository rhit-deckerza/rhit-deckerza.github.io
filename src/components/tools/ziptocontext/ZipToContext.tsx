...
const ZipToContext: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage("Error uploading file: " + (error.response?.data?.error || "Unknown error"));
    } finally {
  ...
  // Get all content from selected items
  ...
  // Render a single zip item with indentation
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ textAlign: 'center', mb: 4 }}>
        ...
        <Paper elevation={0} sx={{ p: 3, mb: 3, border: '1px solid black', textAlign: 'left' }}>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 4, border: '1px solid black' }}>
        <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
          ...
          <Button
            ...
            sx={{ minWidth: '150px' }}
          >
            {isUploading ? 'Uploading...' : zipStructure.length > 0 ? 'Refresh' : 'Upload ZIP'}
          </Button>
        </Stack>
      ...
      {errorMessage && (
        <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: '#fff4f4', border: '1px solid black' }}>
